...
import { Users, TrendingUp, FileText, MessageSquare, ChevronRight } from 'lucide-react';
...
// ── CohortOverview ─────────────────────────────────────────────
export function CohortOverview({ stats }: { stats: CohortStats }) {
  const skillGapData = Object.entries(stats.skill_gaps)
    .sort(([, a], [, b]) => b - a)
    .slice(0, 8)
...
// ── TrainingPlanView ─────────────────────────────────────────────
export function TrainingPlanView({ plan }: { plan: TrainingWeek[] }) {
  return (
    <div>
            ...
            <tbody>
              {rows.map((s) => (
                <tr key={String(s.id)} className="border-b border-border/50 last:border-0">
                  ...
                  <td className="py-2.5">
                    <span className="truncate max-w-32 block text-muted-foreground">{String(s.target_role ?? '—')}</span>
                  </td>
